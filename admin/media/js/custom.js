$(function(){
	...
	function hideNode(node) {
		...
		$.ajax({
			...
			success : function (r) {
				if(r.type=="success"){
					var icon = $(node).find("ins")[0].outerHTML;
					var a = $(node).find("a:first");
					if(r.data.hidden == 1){
						a.html(icon+'<i>'+r.data.menu_title+'</i>');
					}else{
						a.html(icon+r.data.menu_title);
					}
				}
			},
			"dataType":"json"
		...
		$(".menu section").jstree({
			...
			"types" : {
				...
				"types" : {
					...
					"step.root": {
						"icon" : {
							"image" : "/js/jstree/themes/default/icon-folder.png"
						},
						"delete_node" : false,
						"remove" : false,
					...
					"step.leaf": {
						"icon" : {
							"image" : "/js/jstree/themes/default/icon-blue-document-text-image.png"
						},
						"valid_children" : "none",
						"delete_node" : false,
						"remove" : false,
					...
					"term.root": {
						"icon" : {
							"image" : "/js/jstree/themes/default/icon-blue-document-attribute.png"
						},
						"delete_node" : false,
						"remove" : false,
						"valid_children" : "none"
